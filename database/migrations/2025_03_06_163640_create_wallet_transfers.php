...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('wallet_transfers', function (Blueprint $table) {
            ...
            $table->foreignId('wallet_id')->constrained('wallets')->comment('월렛 번호');
            $table->enum('type', ['deposit', 'withdrawal'])->comment('거래 타입');
            $table->decimal('amount', 20, 9)->default(0)->comment('거래금액');
            $table->decimal('fee', 20, 9)->default(0)->comment('수수료');
            $table->decimal('actual_amount', 20, 9)->default(0)->comment('실제금액');
